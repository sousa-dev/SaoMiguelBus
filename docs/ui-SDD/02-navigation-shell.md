...
- **Smart menu.** Actions are route-aware: static per-route actions come from `getStaticActions(pathname)` ([`lib/fab-registry.ts`](../../lib/fab-registry.ts)); stateful screens inject their own via `useFabActions()` ([`lib/fab-store.ts`](../../lib/fab-store.ts)) while focused (e.g. traffic report + center map + scheduled list; marketplace add listing on index, contact/review on detail). A permanent **"Send feedback"** action is always appended last. Full per-screen matrix: [`docs/plans/2026-06-03-002-feat-smart-fab-per-screen-actions-plan.md`](../plans/2026-06-03-002-feat-smart-fab-per-screen-actions-plan.md) § Per-Screen Action Matrix.
- **Collapsed icon.** Shows the first contextual action's icon when present; otherwise the current module icon from `getModuleIcon(pathname)` (see `lib/modules.tsx`); `Zap` only as last resort.
