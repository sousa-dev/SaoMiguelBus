...
1. ~~**Mobile distribution:** keep Play Store listing (`com.hsousa_apps.Autocarros`) and update to Expo, or new app?~~ **Resolved:** Android ships as an **in-place update** — keep package `com.hsousa_apps.Autocarros` (pinned in `app.json` → `android.package`); rebrand display name **São Miguel Bus → São Miguel Hub** only. iOS is a **new App Store listing** with bundle id `com.sousadev.saomiguelhub` (`app.json` → `ios.bundleIdentifier`).
