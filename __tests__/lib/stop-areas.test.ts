 ...
 * (`__tests__/fixtures/azoresbus/stops.canonical.json`, generated by the API's
 * `manage.py dump_canonical_stops` and copied byte-identically from the API
 ...
 * already-collapsed 814 names by a shared VILLAGE PREFIX, for search only.
 *
 * The CANONICAL fixture, not the raw one, because that is what `Stop.name`
 * holds and what the offline bundle ships. The API canonicalizes upstream's
 * inconsistent spellings at import (`P. DELGADA` / `PONTA DELGADA` were two
 * villages to this rule). Testing raw names here would leave these assertions
 * green while pinning a name domain production no longer serves. Never
...
function realStopNames(): string[] {
  const raw = JSON.parse(
    readFileSync(
      join(process.cwd(), '__tests__', 'fixtures', 'azoresbus', 'stops.canonical.json'),
      'utf8',
    ),
  ) as UpstreamStop[];
...
describe('groupStopsIntoAreas — real data, pinned against the shared fixture', () => {
  ...
  it('matches the backend’s real counts: 814 names, 79 areas, 778 covered', () => {
    assert.equal(names.length, 814);
    assert.equal(areas.size, 79);
    const covered = [...areas.values()].reduce((sum, members) => sum + members.length, 0);
    assert.equal(covered, 778);
  });

  it('Capelas has 37 members', () => {
    assert.equal(areas.get('Capelas')?.length, 37);
  });
